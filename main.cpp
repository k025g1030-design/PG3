...
int main() {
    ...
    auto int2int = Math<int, int>::Min(3, 2);
    std::cout << "最小値: " << int2int << std::endl;

    auto int2float = Math<int, float>::Min(3, 5.5f);
    std::cout << "最小値: " << int2float << std::endl;

    auto int2double = Math<int, double>::Min(4, 3.0);
    std::cout << "最小値: " << int2double << std::endl;

    auto float2float = Math<float, float>::Min(3.0f, 5.5f);
    std::cout << "最小値: " << float2float << std::endl;

    auto float2double = Math<float, double>::Min(7.3f, 1.5);
    std::cout << "最小値: " << float2double << std::endl;

    auto double2double = Math<double, double>::Min(3.0, 2.0);
